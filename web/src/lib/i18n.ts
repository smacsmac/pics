...
const en = {
  ...
  describe: 'describe',
  describePlaceholder: 'a dog on the beach…',
  describeHint: 'Finds photos by what is in them. Runs on your PC, offline.',
  describeResults: (n: number) => `the ${n} best matches`,
  describeOff: 'Not installed',
  describeSetup: 'Search by description',
  describeSetupHint: 'Photon can find photos by describing them — « a dog on the beach », « birthday cake ». It needs a model to be downloaded once (about 170 MB). Nothing leaves your PC afterwards, and nothing is sent anywhere.',
  describeInstall: 'Download the model',
  describeInstalling: 'Downloading…',
  describeRemove: 'Remove the model',
  describeIndexing: (done: number, total: number) => `Indexing photos — ${done} of ${total}`,
  describeReady: (n: number) => `${n} photos indexed`,
  describeNoRuntime: 'Needs onnxruntime-node: run « npm install onnxruntime-node » then restart Photon.',
  describeFailed: 'The model could not be loaded',
...
const fr: Dict = {
  ...
  describePlaceholder: 'un chien sur la plage…',
  describeHint: 'Trouve les photos d’après ce qu’on y voit. Tourne sur votre PC, hors ligne.',
  describeResults: (n: number) => `les ${n} photos qui correspondent le mieux`,
  describeOff: 'Non installé',
  describeSetup: 'Recherche par description',
  describeSetupHint: 'Photon peut retrouver des photos en les décrivant — « un chien sur la plage », « gâteau d’anniversaire ». Il lui faut un modèle, à télécharger une seule fois (environ 170 Mo). Ensuite plus rien ne sort de votre PC, et rien n’est envoyé nulle part.',
  describeInstall: 'Télécharger le modèle',
  describeInstalling: 'Téléchargement…',
  describeRemove: 'Retirer le modèle',
  describeIndexing: (done: number, total: number) => `Analyse des photos — ${done} sur ${total}`,
  describeReady: (n: number) => `${n} photos analysées`,
  describeNoRuntime: 'Il manque onnxruntime-node : lancez « npm install onnxruntime-node » puis redémarrez Photon.',
  describeFailed: 'Le modèle n’a pas pu être chargé',
...
const ko: Dict = {
  ...
  describe: '설명',
  describePlaceholder: '해변의 강아지…',
  describeHint: '사진에 무엇이 있는지로 찾습니다. PC에서, 오프라인으로 동작합니다.',
  describeResults: (n: number) => `가장 잘 맞는 사진 ${n}장`,
  describeOff: '설치되지 않음',
  describeSetup: '설명으로 검색',
  describeSetupHint: '« 해변의 강아지 », « 생일 케이크 »처럼 설명으로 사진을 찾을 수 있습니다. 모델을 한 번만 내려받아야 합니다(약 170 MB). 그 뒤로는 PC 밖으로 아무것도 나가지 않습니다.',
  describeInstall: '모델 내려받기',
  describeInstalling: '내려받는 중…',
  describeRemove: '모델 삭제',
  describeIndexing: (done: number, total: number) => `사진 분석 중 — ${total}장 중 ${done}장`,
  describeReady: (n: number) => `사진 ${n}장 분석 완료`,
  describeNoRuntime: 'onnxruntime-node가 필요합니다. « npm install onnxruntime-node » 실행 후 Photon을 다시 시작하세요.',
  describeFailed: '모델을 불러오지 못했습니다',
